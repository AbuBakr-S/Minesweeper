...
var Board = exports.Board = function () {
  ...
  _createClass(Board, [{
    ...
    value: function getNumberOfSurroundingBombs(flipRow, flipColumn) {
      ...
      var numberOfRows = this._bombBoard.length; // The array structure: [[row], [column]] bombBoard(3,3) would have a length of 3
      var numberOfColumns = this._bombBoard[0].length;
      var numberOfSurroundingBombs = 0; // Count the number of adjacent bombs to a flipped tile

      neighbourOffsets.forEach(function (offset) {
        // Return the number of bombs in an adjacent neighbor
        var neighbourRowIndex = flipRow + offset[0]; // Go to current row and add offset to check neighbour tile
        var neighbourColumnIndex = flipColumn + offset[1]; // Go to current column and add offset to check neighbour tile
